...
interface PriceChartProps {
  data: { time: string; poolPrice: number; pythPrice: number | null }[];
  lowerBound: number;
  upperBound: number;
}
    ...
    <div className="glass-panel price-chart-panel" style={{ minHeight: '380px', maxHeight: '420px', display: 'flex', flexDirection: 'column' }}>
      <div className="price-chart-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', fontSize: '0.8rem' }}>
          ...
          {hasPythData && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '20px', height: '3px', background: '#f97316', borderRadius: '2px' }}></div>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          {currentPoolPrice > 0 && (
            <div style={{
              ...
              <div style={{ fontWeight: 700, color: priceChange >= 0 ? 'var(--success)' : 'var(--danger)', fontSize: '1.1rem' }}>
                {currentPoolPrice.toFixed(4)} USDC
              </div>
              <div style={{ fontSize: '0.8rem', fontWeight: 600, color: priceChange >= 0 ? 'var(--success)' : 'var(--danger)', marginTop: '2px' }}>
          ...
          {lowerBound > 0 && upperBound > 0 && (
            <div style={{
              background: 'rgba(88, 166, 255, 0.08)', padding: '8px 14px',
              borderRadius: '8px', border: '1px solid rgba(88, 166, 255, 0.2)', fontSize: '0.8rem',
            }}>
        ...
        {data.length === 0 ? (
          <div style={{ height: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '0.95rem' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.8rem', marginTop: '4px', opacity: 0.7 }}>3秒ごとに自動更新</div>
            </div>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data} margin={{ top: 10, right: 30, left: 10, bottom: 10 }}>
              ...
              {/* 上限レンジ */}
              {upperBound > 0 && (
                <ReferenceLine y={upperBound} stroke="var(--success)" strokeWidth={2} strokeDasharray="4 4"
                  label={{ position: 'top', value: '上限', fill: 'var(--success)', fontSize: 10, fontWeight: 600 }}
                />
              )}

              {/* 下限レンジ */}
              {lowerBound > 0 && (
                <ReferenceLine y={lowerBound} stroke="var(--danger)" strokeWidth={2} strokeDasharray="4 4"
                  label={{ position: 'bottom', value: '下限', fill: 'var(--danger)', fontSize: 10, fontWeight: 600 }}
                />
              )}

              {/* グラデーション塗り（Areaはtooltip除外） */}
              <Area
                type="monotone"
                dataKey="poolPrice"
              ...
              <Line
                ...
                strokeWidth={2.5}
                dot={false}
                activeDot={{ r: 5, stroke: 'var(--accent)', strokeWidth: 2, fill: 'rgba(22, 27, 34, 0.95)' }}
              />

              {/* 市場価格ライン（Pyth） — 同一data配列からpythPriceキーを参照 */}
              {hasPythData && (
                <Line
                  type="monotone"
                  dataKey="pythPrice"
                  ...
                  dot={false}
                  connectNulls
                  activeDot={{ r: 4, stroke: '#f97316', strokeWidth: 2, fill: 'rgba(22, 27, 34, 0.95)' }}
                />
              )}
            </LineChart>
          </ResponsiveContainer>
